...
export const LanguageSearchBox = ({ whichTag }: LanguageSearchBoxProps) => {
  const boxRef = useRef<HTMLDivElement>(null);
  const inputRef = useRef<HTMLInputElement>(null);
  const [isOpen, setisOpen] = useState(false);
  const [languageKeyword, setLanguageKeyword] = useState("");
  const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setLanguageKeyword(event.target.value);
  };
  ...
  return (
    ...
    >
      ...
      />
      {isOpen ? (
        <LanguageList whichTag={whichTag} tagKeyword={languageKeyword} />
      ) : (
        <></>
      )}
    </FlexDiv>
